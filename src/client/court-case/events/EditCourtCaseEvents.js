...
import {editCourtCaseEvents, fetchCourtCase} from "../../store/actions/CourtCaseActions";
import {compose, lifecycle} from "recompose";

const enhance = compose(
    lifecycle({
        componentDidMount: function () {
            const {fetchCourtCase, params: {id}} = this.props;
            fetchCourtCase(id);
        }
    }));
...
const EditCourtCaseEvents = ({caseNumber, defendant, handleSubmit}) => (
    <Grid fluid>
        <PageHeader>Edit Court Case Events <small>Case: {caseNumber} &bull; Defendant {defendant}</small></PageHeader>
...
export default connect(mapStateToProps, {onSubmit: editCourtCaseEvents, fetchCourtCase})(reduxForm({
